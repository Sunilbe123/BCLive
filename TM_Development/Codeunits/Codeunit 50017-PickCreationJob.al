codeunit 50017 "Pick Creation Order to Order"
{
    ...
    var
        ...
        recWarehouseShipmentLineL: Record "Warehouse Shipment Line";
        i: Integer;
        j: Integer;
    begin
        ...
        recWarehouseShipmentLine.SETCURRENTKEY("Source Document", "Source No.", "Qty. Picked");
        ...
        IF recWarehouseShipmentLine.FINDSET THEN Begin
            REPEAT
                ...
                IF DocNo <> recWarehouseShipmentLine."Source No." THEN BEGIN
                    CheckAvailG := true;
                    j := 1;
                    recWarehouseShipmentLineL.Reset();
                    recWarehouseShipmentLineL.SetCurrentKey("Source Document", "Source No.", "Qty. Picked", "Zone Code", "BIN Code"); // MITL.SM.20200503 Indexing correction        
                    recWarehouseShipmentLineL.SETRANGE("Source Document", recWarehouseShipmentLineL."Source Document"::"Sales Order");
                    recWarehouseShipmentLineL.SETRANGE("Source No.", recWarehouseShipmentLine."Source No.");
                    recWarehouseShipmentLineL.SETRANGE("Qty. Picked", 0);
                    recWarehouseShipmentLineL.SETRANGE("Pick Qty.", 0);
                    i := recWarehouseShipmentLineL.COUNT;
                ...
                TotalAvailQtyL := CalcAvailQtyBaseToPick(recWarehouseShipmentLine);
                ...
                ELSE BEGIN
                    ...
                    TempItemTrcBuffRecG.SETRANGE("Item No.", recWarehouseShipmentLine."Item No.");
                    TempItemTrcBuffRecG.SETRANGE("Location Code", recWarehouseShipmentLine."Location Code");
                    TempItemTrcBuffRecG.SETRANGE("Variant Code", recWarehouseShipmentLine."Variant Code");
                    IF NOT TempItemTrcBuffRecG.FINDFIRST THEN BEGIN
                        TempItemTrcBuffRecG."Line No." := TempLineNoL;
                        TempItemTrcBuffRecG."Item No." := recWarehouseShipmentLine."Item No.";
                        TempItemTrcBuffRecG."Location Code" := recWarehouseShipmentLine."Location Code";
                        TempItemTrcBuffRecG."Variant Code" := recWarehouseShipmentLine."Variant Code";
                        TempItemTrcBuffRecG.Quantity := recWarehouseShipmentLine."Qty. Outstanding (Base)";
                ...
                if i = j THEN begin
                    //mitl_6532++
                    if (recWarehouseShipmentLine."Combined Pick") or
                    (recWarehouseShipmentLine."Shipment Date" in [DMY2Date(01, 01, 2000) ..
                    CalendarMgnt.CalcDateBOC(format(SalesReceiveableSetup.PickCreationCalc), Today, 0, 'GB', '', 1, '', '', false)]) Then begin
                        IF CheckAvailG then begin //MITL.AJ.07012020
                            CheckAndRelease;
                            WarehouseShipmentHeader.SETRANGE("No.", recWarehouseShipmentLine."No.");
                            WarehouseShipmentHeader.FINDFIRST;
                            recWarehouseShipmentLineL.SetHideValidationDialogCustom(TRUE);
                            recWarehouseShipmentLineL.CreatePickDocCustom(recWarehouseShipmentLineL, WarehouseShipmentHeader);
                            CreatePickStatusLine(recWarehouseShipmentLineL, StausPickL::"Pick Created");
                        END ELSE BEGIN
                            CreateDetailLines(recWarehouseShipmentLine);
                            CreatePickStatusLine(recWarehouseShipmentLine, StausPickL::"Update in-Progress");
                        END;
                    end;
                end;
        ...
        */
        ...
        PickCombinedPickLines();
        PickOrdertoOrderLines();
    ...
    begin
        ...
        WarehouseShipmentLineL.SetCurrentKey("Source Document", "Source No.", "Qty. Picked", "Zone Code", "BIN Code"); // MITL.SM.20200503 Indexing correction        
    ...
    begin
        ...
        // WhseShipmentLineRecL.SetRange("No.", WhseShipmentHdrRecP."No.");
        ...
        IF WhseShipmentLineRecL.FINDSET THEN
            REPEAT
                /*
                    NoStockLineRecL.Reset();
                    NoStockLineRecL.SetRange("No.", WhseShipmentLineRecL."No.");
                    NoStockLineRecL.SetRange("Line No.", WhseShipmentLineRecL."Line No.");
                    if not NoStockLineRecL.FindFirst() then begin
                        */
    ...
    local procedure PickCombinedPickLines()
    var
        CreateStockEntriesCOD_L: Codeunit "Finding Whse entry";
        NoStockLineRecL: Record "Pick Crt_Whse Shp Lines";
        TempCount: Integer;
    ...
    begin
        ...
        recWarehouseShipmentLine.SETRANGE("Qty. Picked", 0);
        recWarehouseShipmentLine.SETRANGE("Pick Qty.", 0);
        recWarehouseShipmentLine.SetRange("Combined Pick", true);
        IF recWarehouseShipmentLine.FINDSET THEN Begin
            REPEAT
                j := j + 1;
                IF DocNo <> recWarehouseShipmentLine."Source No." THEN BEGIN
                ...
                if i = j THEN begin
                    ...
                    IF CheckAvailG then begin //MITL.AJ.07012020
                        ...
                        CreatePickStatusLine(recWarehouseShipmentLineL, StausPickL::"Pick Created");
                        /*
                        // SM Add-on code 20200603 ++
                        NoStockLineRecL.Reset();
                        NoStockLineRecL.SetRange("Source Subtype", recWarehouseShipmentLineL."Source Subtype");
                        NoStockLineRecL.SetRange("Source No.", recWarehouseShipmentLineL."Source No.");
                        if NoStockLineRecL.FindSet() then
                            NoStockLineRecL.DeleteAll(true);
                        // SM Add-on code 20200603 ++
                        */
                    END ELSE BEGIN
                        CreateDetailLines(recWarehouseShipmentLine);
    ...
    local procedure PickOrdertoOrderLines()
    var
        CreateStockEntriesCOD_L: Codeunit "Finding Whse entry";
        NoStockLineRecL: Record "Pick Crt_Whse Shp Lines";
        TempCount: Integer;
